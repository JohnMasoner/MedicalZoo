...
def compute_average_surface_distance(surface_distances):
    """
    This function computes the average surface distance between two surfaces using the given surface
    distances.

    Args:
      surface_distances: `surface_distances` is a dictionary containing four arrays:

    Returns:
      a tuple containing the average distance from the ground truth surface to the predicted surface and
...
def compute_robust_hausdorff(surface_distances, percent):
    """
    The function computes the robust Hausdorff distance between two surfaces using surface distances and
    a given percentage.

    Args:
      surface_distances: `surface_distances` is a dictionary containing the following keys:
      percent: `percent` is a percentage value (between 0 and 100) that determines the percentage of
    surface points that should be considered when computing the robust Hausdorff distance. For example,
    if `percent` is 95, the function will compute the distance between the 95th percentile of

    Returns:
      The function `compute_robust_hausdorff` returns the maximum value between
    `perc_distance_gt_to_pred` and `perc_distance_pred_to_gt`.
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
...
def compute_surface_overlap_at_tolerance(surface_distances, tolerance_mm):
    """
    This function computes the relative overlap between two surfaces based on their surface distances
    and a given tolerance.

    Args:
      surface_distances: `surface_distances` is a dictionary containing the following keys:
      tolerance_mm: tolerance_mm is a parameter that specifies the maximum distance (in millimeters)
    between the predicted surface and the ground truth surface for which the surfaces are considered to
    overlap. If the distance between a predicted surface point and its nearest ground truth surface
    point is less than or equal to the tolerance_mm,

    Returns:
      a tuple containing two values: the relative overlap of ground truth surfaces and predicted
    surfaces at a given tolerance in millimeters.
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
...
def compute_surface_dice_at_tolerance(surface_distances, tolerance_mm):
    """
    The function computes the surface dice coefficient at a given tolerance level using surface
    distances and surfel areas.

    Args:
      surface_distances: surface_distances is a dictionary containing the following keys:
      tolerance_mm: tolerance_mm is a parameter that specifies the maximum distance (in millimeters)
    between the predicted surface and the ground truth surface for a point to be considered as correctly
    predicted. Points that are farther away than the tolerance_mm value are considered as incorrectly
    predicted. This parameter is used in the computation of

    Returns:
      the surface dice coefficient computed using the input surface distances and tolerance_mm.
    """
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
